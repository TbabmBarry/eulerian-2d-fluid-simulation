...
FluidField::FluidField(System* sys, int grid_N) : sys(sys), N(grid_N), dt(0.01f), diff(0.0f), visc(0.0f) {
    allocate();
}
...
void FluidField::freeData()
{
	...
	if ( u ) free ( u );
	if ( v ) free ( v );
	if ( u_previous ) free ( u_previous );
	if ( v_previous ) free ( v_previous );
	if ( density ) free ( density );
	if ( density_previous ) free ( density_previous );
}
