...
void AngularSpring::apply(bool springsCanBreak)
{
  if (this->active)
  {
    ...
    Vec2f force1 = (m_ks*(norm(p1restlength)-0.2)+m_kd*((p1restlength*length1_derivate)/norm(p1restlength)))*(p1restlength/norm(p1restlength));
    Vec2f force3 = (m_ks*(norm(p3restlength)-0.2)+m_kd*((p3restlength*length3_derivate)/norm(p3restlength)))*(p3restlength/norm(p3restlength));
