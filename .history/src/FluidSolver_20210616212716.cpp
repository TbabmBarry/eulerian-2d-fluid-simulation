...
void FluidSolver::simulateStep(System *system, float h, float* u, float* v){
}
...
void vorticity_confinement(int N, float dt){
    ...
    for (int j = 1; j <= N; j++) {
        for (int i = 1; i < N; i++) {
            ...
            // curlx = dw/dy - dv/dz
            int x = (u[(i+1) + j*n] - this.u[(i-1) + j*n]) * 0.5;
    ...
    for (var j = 1; j <= this.height; j++) {
        for (var i = 1; i < this.width; i++) {
            ...
            Nx *= len1;
            Ny *= len1;
            u[IX(i,j)] += Nx*this.u_prev[IX(i,j)];
            v[IX(i,j)] += Ny*this.v_prev[IX(i,j)];
        }
    }
}
