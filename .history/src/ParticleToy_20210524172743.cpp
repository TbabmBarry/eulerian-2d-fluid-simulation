...
static void init_system(void)
{
	...
	sysEx = new System(new EulerSolver(EulerSolver::EXPLICIT));
	sysIm = new System(new EulerSolver(EulerSolver::IMPLICIT));
	mode = new Mode();
...
static void key_func ( unsigned char key, int x, int y )
{
	...
	case '1':
		init_system();
		// free_data();
		printf ( "\t key '1' for Spring force in semi solver\n" );
		printf ( "\t key '1' for Spring force in explicit solver\n" );
		printf ( "\t key '1' for Spring force in implicit solver\n" );
		sys->dt=0.1;
		mode->Spring(sys);
		switch ( key ){
			case '1':
