...
void Mode::hair(System *sys){
	const Vec2f center(0.0, 0.5f);

    const int numHairs = 1;
    // vector<Vec2f>
    Vec2f end(0.0, -0.5f);
    Vec2f start(0.0, 0.5f);
    const int num_particles = 10;
    Vec2f step=(start-end)/(num_particles+1);
    ...
    for (int i = 0; i < numHairs; i++) {
        ...
        for (int j = 0; j < num_particles+2; j++) {
            sys->addParticle(new Particle(start-step * j, 0.1f, i * num_particles + j));
        }
        ...
        for (int j = 1; j < num_particles+1; j++) {
            sys->addForce(new AngularSpring(sys->particles[i * num_particles + j - 1],
											...
											sys->particles[i * num_particles + j + 1],
											180*num_particles/(num_particles+2), ks, kd));
        }
        sys->addConstraint(new FixedPointConstraint(sys->particles[i * num_particles], center));
        // sys->addConstraint(new CircularWireConstraint(sys->particles[0], center, 0.1));
    }
